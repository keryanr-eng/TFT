...
export function Board({ board, cells, phase, opponentLabel, highlightedUnitIds, selectedItemId, onDropUnit, onDropItemToUnit, onHoverUnit, }) {
    ...
    return (_jsxs("section", { className: "flex h-full min-h-0 flex-col rounded-[2rem] border border-black/10 bg-white/56 p-3 shadow-[0_26px_70px_rgba(31,37,48,0.16)] backdrop-blur", children: [_jsx("div", { className: "mb-2 flex items-center justify-between px-1", children: _jsx("p", { className: "truncate text-[11px] font-semibold uppercase tracking-[0.24em] text-ink/38", children: opponentLabel }) }), _jsx("div", { className: "panel-grid flex-1 overflow-hidden rounded-[1.9rem] border border-black/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.78)_0%,rgba(255,247,231,0.93)_100%)] p-3", children: _jsx("div", { className: "grid h-full gap-2", style: {
                        ...
                        return (_jsx("div", { className: clsx("min-h-0 rounded-[1.4rem] p-1 transition", side === "away" ? "bg-white/28" : "bg-white/46", isDropTarget && "hover:bg-lagoon/5", visualRow === board.awayRows - 1 && "border-b border-b-lagoon/18"), onDragOver: isDropTarget
                                ? (event) => {
                                ...
                                : undefined, children: unit ? (_jsx(BoardUnit, { className: "h-full", draggable: canDrop && side === "home", highlighted: highlightedUnits.has(unit.instanceId), itemIds: unit.itemIds, itemDropState: canReceiveItem
                                    ...
                                    : undefined, unit: unit })) : (_jsx("div", { className: "h-full rounded-[1.2rem] bg-white/20" })) }, `${side}-${row}-${column}`));
                    })) }) })] }));
}
