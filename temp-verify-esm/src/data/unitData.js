...
export const unitData = unitRows.map((row) => {
    const cost = Math.min(5, Math.max(1, toNumber(row.cost, 1)));
...
export const unitById = Object.fromEntries(unitData.map((unit) => [unit.id, unit]));
export const unitIdsByCost = unitData.reduce((accumulator, unit) => {
