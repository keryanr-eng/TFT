        ...
        default: {
            if (target) {
                ...
                if (description.includes("repositionne") || description.includes("se repositionne")) {
                    const instruction = buildRetreatInstruction(caster, target, getOccupiedPositions(combat, caster.id), board);
                    if (instruction) {
                        caster.position = instruction.to;
                        caster.side = instruction.to.side;
                    }
...
export function stepCombat(combat, deltaMs, board, activeSynergiesByPlayer) {
    ...
    for (const unit of units) {
        ...
        if (unit.moveTimerMs <= 0) {
            const instruction = buildMovementInstruction(unit, target, getOccupiedPositions(combat, unit.id), board);
            if (instruction) {
                unit.position = instruction.to;
