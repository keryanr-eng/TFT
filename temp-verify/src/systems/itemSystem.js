...
exports.createEmptyHoverStatsSnapshot = createEmptyHoverStatsSnapshot;
exports.createStarterItemInventory = createStarterItemInventory;
exports.combineItems = combineItems;
exports.resolveItemCombinations = resolveItemCombinations;
exports.buildBaseResolvedStats = buildBaseResolvedStats;
exports.applyItemBonusesToStats = applyItemBonusesToStats;
exports.buildResolvedStatsWithItems = buildResolvedStatsWithItems;
exports.equipItemOnUnit = equipItemOnUnit;
exports.collapseItemsForUpgrade = collapseItemsForUpgrade;
exports.getMaxUnitItemSlots = getMaxUnitItemSlots;
...
function createEmptyHoverStatsSnapshot() {
    return {
        maxHealth: 0,
        maxMana: 0,
        startingMana: 0,
...
function isComponentItem(itemId) {
    return itemData_1.itemById[itemId]?.kind === "component";
}
function createStarterItemInventory() {
    return itemData_1.itemComponents.slice(0, 5).map((item) => item.id);
}
function combineItems(componentA, componentB) {
    const left = itemData_1.itemById[componentA];
    const right = itemData_1.itemById[componentB];
    if (!left || !right || left.kind !== "component" || right.kind !== "component") {
        return null;
    }
    const sortedPair = [left.id, right.id].sort().join(":");
    return itemData_1.itemRecipes.find((recipe) => recipe.recipe.slice().sort().join(":") === sortedPair) ?? null;
}
function resolveItemCombinations(itemIds) {
...
function buildBaseResolvedStats(template, starLevel) {
    return {
...
function applyItemBonusesToStats(baseStats, itemIds) {
...
function buildResolvedStatsWithItems(template, starLevel, itemIds) {
    return applyItemBonusesToStats(buildBaseResolvedStats(template, starLevel), itemIds);
}
function equipItemOnUnit(unit, itemId) {
