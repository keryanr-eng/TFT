...
exports.planBotTurn = planBotTurn;
exports.runBotPrepTurn = runBotPrepTurn;
...
function scoreOffer(player, template) {
...
function getRolePriority(template) {
...
function placeUnitsForBot(game, playerId) {
    const player = game.players[playerId];
    if (!player) {
        return;
    }
    const maxUnits = (0, levelSystem_1.getMaxDeployableUnits)(player.experience.level);
    const units = player.rosterUnitIds
        .map((unitId) => ({
        unitId,
        instance: game.unitsById[unitId],
        template: unitData_1.unitById[game.unitsById[unitId]?.templateId ?? ""],
    }))
        .filter((entry) => Boolean(entry.instance && entry.template))
        .sort((left, right) => {
    ...
    for (const entry of selected) {
        const frontliner = entry.template.roleArchetype === "tank" || entry.template.roleArchetype === "bruiser";
    ...
    (0, gameMutations_1.recalculatePlayerSynergies)(game, playerId);
}
function planBotTurn(bot, shopPool) {
...
function runBotPrepTurn(game, playerId) {
    const player = game.players[playerId];
    if (!player || player.isEliminated) {
        return;
    }
    if (player.experience.level < 8 && player.economy.gold >= 20) {
        (0, gameMutations_1.buyExperienceForPlayer)(game, playerId);
    }
    for (let actionIndex = 0; actionIndex < 6; actionIndex += 1) {
        const scoredOffers = player.shop.offers
            .map((offer) => ({
            offer,
            template: unitData_1.unitById[offer.unitId],
        }))
            .filter((entry) => Boolean(entry.template && entry.offer.cost <= player.economy.gold))
            .map((entry) => ({
            ...entry,
        ...
        if (scoredOffers.length === 0) {
            if (player.economy.gold >= player.shop.rerollCost + 2 && actionIndex < 2) {
                (0, gameMutations_1.refreshPlayerShop)(game, playerId, true);
                continue;
            }
            break;
        }
        ...
        (0, gameMutations_1.purchaseShopOffer)(game, playerId, best.offer.slotIndex);
