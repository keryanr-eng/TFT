...
import type { GameState, PlayerState, UnitInstanceId, UnitStarLevel } from "../types/gameTypes";
...
function getPlacementPriority(player: PlayerState, unitId: UnitInstanceId): number {
...
function getNextStarLevel(starLevel: UnitStarLevel): UnitStarLevel {
  if (starLevel === 1) {
    return 2;
  }
...
export function resolveAutomaticUpgrades(game: GameState, playerId: string): void {
  ...
  while (merged) {
    ...
    const groups = new Map<string, UnitInstanceId[]>();
    ...
    for (const group of groups.values()) {
      ...
      const sorted = [...group].sort(
        (left, right) => getPlacementPriority(player, left) - getPlacementPriority(player, right),
      );
