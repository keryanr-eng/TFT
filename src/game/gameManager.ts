...
function appendLog(game: GameState, tone: LogTone, text: string): void {
  game.log.unshift({
...
function updateStreak(player: PlayerState, won: boolean): void {
...
function getLivingBotIds(game: GameState): PlayerId[] {
...
function estimateArmyPower(game: GameState, playerId: PlayerId): number {
  const player = game.players[playerId];
  if (!player) {
    return 0;
  }
  ...
  const synergyPower = (game.activeSynergiesByPlayer[playerId] ?? []).reduce(
    (total, synergy) => total + (synergy.activeTier ?? 0) * 6 + synergy.count,
    0,
  );
...
function resolveOffscreenBotBattles(game: GameState, excludedIds: Set<PlayerId>): void {
...
function chooseOpponentId(game: GameState, roundIndex: number): PlayerId {
...
function setupNeutralArmy(game: GameState, roundIndex: number): void {
  ...
  for (const member of formation) {
    ...
    const targetSlot = neutral.boardSlots.find(
      (slot) => slot.row === member.position.row && slot.column === member.position.column,
    );
    if (targetSlot) {
...
function clearNeutralArmy(game: GameState): void {
  clearPlayerArmy(game, game.neutralPlayerId);
}

function createMatchup(game: GameState, opponentId: PlayerId): MatchupState {
  return {
...
function prepareRound(game: GameState, initial: boolean): void {
  if (!initial) {
  ...
  appendLog(
    game,
    "info",
    `Round ${game.round.stageLabel} - ${game.round.kind.toUpperCase()} contre ${game.players[opponentId].name}.`,
  );
  evaluateWinner(game);
}
...
export function moveHumanUnit(
  ...
  const moved = movePlayerUnit(
    human,
    unitId,
    destination,
    getMaxDeployableUnits(human.experience.level),
  );
...
export function rerollHumanShop(current: GameState): GameState {
  const next = cloneGame(current);
  if (next.phase !== "prep") {
    return next;
  }
  ...
  if (rerolled) {
    appendLog(
      next,
      "info",
      `Shop relance. ${next.players[next.humanPlayerId].economy.gold}g restants.`,
    );
  }
  return next;
}

export function buyHumanUnit(current: GameState, slotIndex: number): GameState {
  const next = cloneGame(current);
  if (next.phase !== "prep") {
    return next;
  }
  ...
  if (purchased) {
    appendLog(
      next,
      "success",
      `${unitName} achetee. ${next.players[next.humanPlayerId].economy.gold}g restants.`,
    );
  }
...
export function buyHumanExperience(current: GameState): GameState {
  const next = cloneGame(current);
  if (next.phase !== "prep") {
    return next;
  }
  ...
  if (bought) {
    ...
    appendLog(
      next,
      "info",
      `Experience achetee. Niveau ${human.experience.level}, ${human.economy.gold}g restants.`,
    );
  }
...
export function equipHumanItem(current: GameState, itemId: string, unitId: string): GameState {
  const next = cloneGame(current);
  if (next.phase !== "prep") {
    return next;
  }
...
export function startCombatPhase(current: GameState): GameState {
  const next = cloneGame(current);
...
export function tickCombatPhase(current: GameState, deltaMs: number): GameState {
  if (current.phase !== "combat" || !current.combat) {
    return current;
  }

  const next = cloneGame(current);
  next.combat = stepCombat(
    next.combat!,
    deltaMs,
    next.board,
    next.activeSynergiesByPlayer,
  );
...
export function advanceAfterResolution(current: GameState): GameState {
  const next = cloneGame(current);
